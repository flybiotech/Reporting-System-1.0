...
public class PrintPluginStatusHelper {
    //这些都是包名
    public static final String MOPRIA_PRINT_SERVICE_NAME  = "org.mopria.printplugin.MopriaPrintService";//mopria打印插件服务
    public static final String MOPRIA_PRINT_PLUGIN_PACKAGE_NAME = "org.mopria.printplugin";//mopria联盟打印插件
    public static final String BROTHER_PRINT_PLUGIN_PACKAGE_NAME = "com.brother.printservice";//兄弟公司打印插件
    public static final String SAMSUNG_PRINT_PLUGIN_PACKAGE_NAME = "com.sec.app.samsungprintservice";//三星打印插件
    public static final String OTHER_PRINT_PLUGIN_PACKAGE_NAME = "Other Print Service Plugin";//其他打印插件
...
//    private static final String[] pluginTencentUrls    = {"http://sj.qq.com/myapp/search.htm?kw=HP%20Print%20Service%20plugin"};
    //这个是自己修改的
    private static final String[] pluginTencentUrls    = {
       "http://www.flybiotech.com/uploadfiles/HPprintservice.apk",
    "http://www.flybiotech.com/uploadfiles/mopria print service.apk",
    "http://www.flybiotech.com/uploadfiles/Samsung Print Service.apk"
    };
    /**
     * Plugin Icon Name
     */
    ...
    private Map<String, PrintPlugin> createPluginVersionMap() {
        //获取打印服务插件的名称 e ：惠普打印服务插件， Mopria Print Service
        String[] pluginNames = context.getResources().getStringArray(R.array.plugin_names);
        //获取提供打印服务插件的公司的名称 例如:惠普公司 ,Mopria联盟
        ...
        if (isChina()) {
            for(int i=0; i < pluginTencentUrls.length; i++) {
                /**
                 * params: 第一个参数： 包名             第二个参数 ：有用插件的最低版本号，   第三个参数：playstore下载地址
                 * 第四个参数： context    第五个参数 ：显示名称   ，第六个参数：打印插件的制造(商)  第七个参数： 图标
                 */
...
//                再次检查插件的状态
    ...
    private void bubbleSort(ArrayList<PrintPlugin> arrayList) {
        ...
        sortingMap.put("com.sec.app.samsungprintservice", 3);
        sortingMap.put("com.brother.printservice", 4);
    ...
    public boolean goToGoogleStore(PrintPlugin printPlugin) {
        ...
        return (pluginStatus == PrintPlugin.PluginStatus.NOTINSTALLED ||//未安装状态
                pluginStatus == PrintPlugin.PluginStatus.REQUIREUPDATE || //更新
                pluginStatus == PrintPlugin.PluginStatus.DOWNLOADING);  //正在下载
    }
