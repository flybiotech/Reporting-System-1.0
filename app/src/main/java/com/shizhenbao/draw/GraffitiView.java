...
import static com.shizhenbao.draw.DrawUtil.computeAngle;
import static com.shizhenbao.draw.DrawUtil.drawArrow;
import static com.shizhenbao.draw.DrawUtil.drawCircle;
import static com.shizhenbao.draw.DrawUtil.drawLine;
import static com.shizhenbao.draw.DrawUtil.drawRect;
...
*/
    ...
    private void draw(Canvas canvas, Pen pen, Shape shape, Paint paint, float sx, float sy, float dx, float dy, Matrix matrix, GraffitiColor color, int degree) {
        ...
        switch (shape) { // 绘制图形
            ...
            case FILL_CIRCLE:
                paint.setStyle(Paint.Style.FILL);
                drawCircle(canvas,sx,sy,30,paint);
            case HOLLOW_CIRCLE:
...
//            }
    ...
    public void undo() {
        if (mUndoStack.size() > 0) {
            ...
            draw(mBitmapCanvas, mPathStack);
            invalidate();
            if(OneItem.getOneItem().getTab()>=1){
                OneItem.getOneItem().setTab(OneItem.getOneItem().getTab()-1);
            }
//            Toast.makeText(getContext(), "标记："+OneItem.getOneItem().getTab(), Toast.LENGTH_SHORT).show();
        }
    }
