...
void build_content(char* p, int n)
{
  ...
  while (n > 0)
  {
    k = snprintf(line_buffer, sizeof(line_buffer), "%04d_ABCDEFGHIJKL_This_line_is_64_bytes_long_ABCDEFGHIJKL_%04d\r\n", i, i);
    if (k != 64)
    {
...
void handle_http_connection(int c)
{
  ...
  {
    n = read(c, request_buffer + request_len, sizeof(request_buffer) - request_len);
    if (n < 0)
    {
      if (errno == EINTR)
        continue;
  ...
  switch (mode)
  {
    ...
    case MODE_ADDCRAP:
      ...
      if (!response_buffer)
        break;
      n = snprintf(response_buffer, response_len, "HTTP/1.1 200 OK\r\n""Server: minihttptestserver\r\n""Content-Type: text/plain\r\n""Content-Length: %d\r\n""\r\n", content_length);
...
int main(int argc, char** argv)
{
  ...
  if (bind(s, (struct sockaddr*) &server_addr, ipv6 ? sizeof(struct sockaddr_in6): sizeof(struct sockaddr_in)) < 0)
  {
    perror("bind");
    return 1;
  } if (listen(s, 5) < 0)
