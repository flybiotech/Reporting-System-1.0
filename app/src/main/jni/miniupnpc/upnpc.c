...
{
  static const char proto_tcp[4] = 
  {
    'T', 'C', 'P', 0
  };
  static const char proto_udp[4] = 
  {
    'U', 'D', 'P', 0
  };
  int i, b;
...
static void DisplayInfos(struct UPNPUrls* urls, struct IGDdatas* data)
{
  ...
  if (UPNP_GetConnectionTypeInfo(urls->controlURL, data->first.servicetype, connectionType) != UPNPCOMMAND_SUCCESS)
    printf("GetConnectionTypeInfo failed.\n");
  else
    printf("555 Connection Type : %s\n", connectionType);
  ...
  if (UPNP_GetStatusInfo(urls->controlURL, data->first.servicetype, status, &uptime, lastconnerr) != UPNPCOMMAND_SUCCESS)
    ...
    printf("666 Status : %s, uptime=%us, LastConnectionError : %s\n", status, uptime, lastconnerr);

  timenow = time(NULL);
  timestarted = timenow - uptime;
  printf("  Time started : %s", ctime(&timestarted));

  if (UPNP_GetLinkLayerMaxBitRates(urls->controlURL_CIF, data->CIF.servicetype,  &brDown, &brUp) != UPNPCOMMAND_SUCCESS)
  {
...
static void ListRedirections(struct UPNPUrls* urls, struct IGDdatas* data)
{
  ...
  do
  {
    ...
    intPort[0] = '\0';
    intClient[0] = '\0';
    r = UPNP_GetGenericPortMappingEntry(urls->controlURL, data->first.servicetype, index, extPort, intClient, intPort, protocol, desc, enabled, rHost, duration);
    if (r == 0)
      /*
...
static void NewListRedirections(struct UPNPUrls* urls, struct IGDdatas* data)
{
  ...
  r = UPNP_GetListOfPortMappings(urls->controlURL, data->first.servicetype, "0", "65535", "TCP", "1000",  &pdata);
  if (r == UPNPCOMMAND_SUCCESS)
  {
    printf(" i protocol exPort->inAddr:inPort description remoteHost leaseTime\n");
    for (pm = pdata.l_head; pm != NULL; pm = pm->l_next)
    {
      printf("%2d %s %5hu->%s:%-5hu '%s' '%s' %u\n", i, pm->protocol, pm->externalPort, pm->internalClient, pm->internalPort, pm->description, pm->remoteHost, (unsigned)pm->leaseTime);
      i++;
  ...
  r = UPNP_GetListOfPortMappings(urls->controlURL, data->first.servicetype, "0", "65535", "UDP", "1000",  &pdata);
  if (r == UPNPCOMMAND_SUCCESS)
  {
    for (pm = pdata.l_head; pm != NULL; pm = pm->l_next)
    {
...
static void SetRedirectAndTest(struct UPNPUrls* urls, struct IGDdatas* data, const char* iaddr, const char* iport, const char* eport, const char* proto, const char* leaseDuration, const char* description, int addAny)
{
  char externalIPAddress[40];
  char intClient[40];
  char intPort[6];
  ...
  if (addAny)
  {
    r = UPNP_AddAnyPortMapping(urls->controlURL, data->first.servicetype, eport, iport, iaddr, description, proto, 0, leaseDuration, reservedPort);
    if (r == UPNPCOMMAND_SUCCESS)
      eport = reservedPort;
    else
  ...
  {
    r = UPNP_AddPortMapping(urls->controlURL, data->first.servicetype, eport, iport, iaddr, description, proto, 0, leaseDuration);
    if (r != UPNPCOMMAND_SUCCESS)
      printf("AddPortMapping(%s, %s, %s) failed with code %d (%s)\n", eport, iport, iaddr, r, strupnperror(r));
  }

  r = UPNP_GetSpecificPortMappingEntry(urls->controlURL, data->first.servicetype, eport, proto, NULL /*remoteHost*/, intClient, intPort, NULL /*desc*/, NULL /*enabled*/, duration);
  if (r != UPNPCOMMAND_SUCCESS)
...
static void RemoveRedirect(struct UPNPUrls* urls, struct IGDdatas* data, const char* eport, const char* proto, const char* remoteHost)
{
  int r;
  if (!proto || !eport)
  {
...
static void RemoveRedirectRange(struct UPNPUrls* urls, struct IGDdatas* data, const char* ePortStart, char const* ePortEnd, const char* proto, const char* manage)
{
  int r;
...
int main(int argc, char** argv)
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    if (argv[i][0] == '-')
    {
      ...
      {
        char junk;
        if (sscanf(argv[++i], "%d%c", &localport, &junk) != 1 || localport < 0 || localport > 65535 || (localport > 1 && localport < 1024))
        {
  ...
  if (!command || (command == 'a' && commandargc < 4) || (command == 'd' && argc < 2) || (command == 'r' && argc < 2) || (command == 'A' && commandargc < 6) || (command == 'U' && commandargc < 2) || (command == 'D' && commandargc < 1))
  {
  ...
  if (rootdescurl || (devlist = upnpDiscover(2000, multicastif, minissdpdpath, localport, ipv6, ttl, &error)))
  {
    ...
    if (devlist)
    {
      printf("111:List of UPNP devices found on the network :\n");
    ...
    if ((rootdescurl && UPNP_GetIGDFromUrl(rootdescurl, &urls, &data, lanaddr, sizeof(lanaddr))) || (i = UPNP_GetValidIGD(devlist, &urls, &data, lanaddr, sizeof(lanaddr))))
    {
      ...
      printf("444 Local LAN ip address : %s\n", lanaddr);
#if 0
      printf("getting \"%s\"\n", urls.ipcondescURL);
      ...
      case 'L':
        ...
        break;
      case 'a':
        SetRedirectAndTest(&urls, &data, commandargv[0], commandargv[1], commandargv[2], commandargv[3], (commandargc > 4) ? commandargv[4]: "0", description, 0);
        break;
      case 'd':
        RemoveRedirect(&urls, &data, commandargv[0], commandargv[1], commandargc > 2 ? commandargv[2]: NULL);
        break;
      case 'n':
        /* aNy */
        SetRedirectAndTest(&urls, &data, commandargv[0], commandargv[1], commandargv[2], commandargv[3], (commandargc > 4) ? commandargv[4]: "0", description, 1);
        break;
      case 'N':
        if (commandargc < 3)
          fprintf(stderr, "too few arguments\n");

        RemoveRedirectRange(&urls, &data, commandargv[0], commandargv[1], commandargv[2], commandargc > 3 ? commandargv[3]: NULL);
        break;
      case 's':
      ...
      case 'r':
        ...
        while (i < commandargc)
        {
          ...
          else if (is_int(commandargv[i + 1]))
          {
            ...
            SetRedirectAndTest(&urls, &data, lanaddr, commandargv[i], commandargv[i + 1], commandargv[i + 2], "0", description, 0);
            i += 3; /* 3 parameters parsed */
          }
          else
          {
            /* 2nd parameter not an integer : <port> <protocol> */
            SetRedirectAndTest(&urls, &data, lanaddr, commandargv[i], commandargv[i], commandargv[i + 1], "0", description, 0);
            i += 2; /* 2 parameters parsed */
          }
        }
        break;
      case 'A':
        SetPinholeAndTest(&urls, &data, commandargv[0], commandargv[1], commandargv[2], commandargv[3], commandargv[4], commandargv[5]);
        break;
      case 'U':
      ...
      case 'S':
        ...
        break;
      case 'G':
        GetPinholeOutboundTimeout(&urls, &data, commandargv[0], commandargv[1], commandargv[2], commandargv[3], commandargv[4]);
        break;
      case 'P':
