...
int test(const char* portListingXml, int portListingXmlLen, const struct port_mapping* ref, int count)
{
  ...
  for (i = 0, pm = data.l_head; (pm != NULL) && (i < count); i++, pm = pm->l_next)
  {
...
{
  int r;
  r = test(test_document, sizeof(test_document) - 1, port_mappings, PORT_MAPPINGS_COUNT);
  if (r == 0)
  {
