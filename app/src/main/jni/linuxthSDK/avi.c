...
-------------------------------------------------------------------------------
*/
typedef struct GmAviFileChunkEntry
{
//length entry
...
*/
  ...
  else if (whence == GMAVI_SEEK_TO_END)
// this is in effect before any writing
  {
    avi_file->strh_pos = avi_file->hdrl_len - sizeof(AviMainHeader) - 4;
    avi_file->data_pos = avi_file->movi_len - 4;
    avi_file->index_pos = avi_file->idx1_len;
...
int GMAVISetStreamHeader(int handle, 
                         AviStreamHeader* avi_stream_header,
                         GmAviStreamFormat* avi_stream_format,
                         int* out_streamid)
{
  GmAviFile* avi_file = (GmAviFile*)handle;
  int size = 0;
...
int GMAVISetStreamDataAndIndex(int handle, int streamid, unsigned char* data, int length, int intra, unsigned char* extra_data, int extra_length)
{
  GmAviFile* avi_file = (GmAviFile*)handle;
  AviIndex index;
...
/*
int GMAVIGetStreamHeaderSize(int handle, int num, int *header_size, int *format_size)
{
GmAviFile *avi_file = (GmAviFile*)handle;
...
int GMAVIGetStreamDataAndIndex(int handle, int* streamid, unsigned char* data, int* length, int* intra, unsigned char* extra_data, int* extra_length, int frame_no, int reverse, int* pos)
{
  GmAviFile* avi_file = (GmAviFile*)handle;
...
int GMAVIFillAudioStreamHeaderValues(AviStreamHeader* header, GmAviStreamFormat* format, int type, int channels, int sample_size, int bitrate)
{
  int fcc_type = 0;
  if (!header)
    return GMSTS_INVALID_INPUT;
